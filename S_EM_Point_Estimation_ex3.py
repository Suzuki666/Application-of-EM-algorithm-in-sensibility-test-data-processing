...
def normal_distribution(x):  # Operate x<0 (Target point is in the left side of the distribution center)处理x<0(目标点在分布中心左侧)的情况
    if x < 0:
        return 1 - normal_distribution(-x)
    if x == 0:
        return 0.5  # Solve the integral of probability density of standard normal distribution 求标准正态分布的概率密度的积分
    s = 1 / 10000
    xk = []
...
# Set the initial value of parameter mu and sigma 2 设置参数mu和sigma2的初值
mu = 0
sigma2 = 0


# Calculate the information dig vaule yi when li == 1, solve integral function named yi1, pack function 计算信息挖掘值yi when li == 1，求积分函数代号为yi1，函数打包
...
# Calculate the information dig vaule yi when li == 0, solve integral function named yi2, pack function 计算信息挖掘值yi when li == 0，求积分函数代号为yi2，函数打包
...
# Calculate the information dig vaule ti when li == 1, solve integral function named ti1, pack function 计算信息挖掘值ti when li == 1，求积分函数代号为ti1，函数打包
...
# Calculate the information dig vaule ti when li == 0, solve integral function named ti2, pack function 计算信息挖掘值ti when li == 0，求积分函数代号为ti2，函数打包
...
if __name__ == '__main__':
    # Input sensitivity test result 导入感度试验结果
    ...
    while abs(Mu_new - Mu_old) + abs(Sigma2_new - Sigma2_old) > 0.0001 and count < 51:
        ...
        for i in range(n):
            if Response[i] == 1:
                # Calculate the information dig value yi and ti when successful response 计算响应成功时的信息挖掘值yi和ti
                Infor_yi[i] = fx_JiFen_yi1(TestCondition[i], Mu_new, Sigma2_new)
                Infor_ti[i] = fx_JiFen_ti1(TestCondition[i], Mu_new, Sigma2_new)
            elif Response[i] == 0:
                # Calculate the information dig value yi and ti when unsuccessful response 计算未成功响应时的信息挖掘值yi和ti
